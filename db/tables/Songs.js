...
async function getByPrefix(prefix,categoryName,db){
    return await new Promise((resolve,reject)=>{
        if (categoryName == "wszystko"){
            db.all("SELECT title,id,category FROM songs WHERE UPPER(title) LIKE UPPER(?) ORDER BY title ASC",[prefix + "%"],(err,rows)=>{
                if (err) {
                    console.error(err.message)
                    reject(err)
                }
            ...
            db.all("SELECT title,id,category FROM songs WHERE UPPER(title) LIKE UPPER(?) AND category=? ORDER BY title ASC",[prefix + "%",categoryName],(err,rows)=>{
                if (err) {
                    console.error(err.message)
                    reject(err)
                }
